import { anyOf, list, map, number, schema, string } from 'v1/schema'
...
describe('parseProjection', () => {
  describe('savedAs attrs', () => {
    ...
    it('correctly parses projection (root)', () => {
      expect(
        parseSchemaProjection(schemaWithSavedAs, ['savedAs']),
      ).toStrictEqual({
    ...
    it('correctly parses projection (nested)', () => {
      expect(
        parseSchemaProjection(schemaWithSavedAs, ['savedAs', 'nested.savedAs']),
      ).toStrictEqual({
    ...
    it('correctly parses projection (with id)', () => {
      expect(
        parseSchemaProjection(schemaWithSavedAs, ['savedAs'], '1'),
      ).toStrictEqual({
    ...
    it('correctly parses condition (listed)', () => {
      expect(
        parseSchemaProjection(schemaWithSavedAs, [
          'savedAs',
          'listed[4].savedAs',
        ]),
      ).toStrictEqual({
  ...
  describe('anyOf', () => {
    ...
    it('correctly parses projection (nested str)', () => {
      expect(
        parseSchemaProjection(schemaWithAnyOf, ['anyOf.str']),
      ).toStrictEqual({
    ...
    it('correctly parses projection (nested num)', () => {
      expect(
        parseSchemaProjection(schemaWithAnyOf, ['anyOf.num']),
      ).toStrictEqual({
        ProjectionExpression: '#p_1.#p_2',

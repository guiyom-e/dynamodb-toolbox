import { list, map, number, schema, string } from 'v1/schema'
...
describe('parseCondition - singleArgFn', () => {
  ...
  it('type', () => {
    expect(
      parseSchemaCondition(simpleSchema, { attr: 'list', type: 'L' }),
    ).toStrictEqual({
  ...
  it('contains (value)', () => {
    expect(
      parseSchemaCondition(simpleSchema, { attr: 'str', contains: 'foo' }),
    ).toStrictEqual({
  ...
  it('beginsWith (value)', () => {
    expect(
      parseSchemaCondition(simpleSchema, { attr: 'str', beginsWith: 'foo' }),
    ).toStrictEqual({
  ...
  it('deep maps and lists (attribute)', () => {
    expect(
      ...
      ConditionExpression:
        'begins_with(#c_1[1].#c_2.#c_3[2].#c_4, #c_5[3].#c_6.#c_7[4].#c_8)',
      ExpressionAttributeNames: {
        '#c_1': 'listA',
        '#c_2': 'nested',
        '#c_3': 'listB',

...
export const $set = <VALUE>(value: VALUE): SET<VALUE> => ({
  [$HAS_VERB]: true,
  [$SET]: value,
})
...
export const $get = <
  ...
  FALLBACK extends
    | undefined
    | AttributeValue
    | Reference<Attribute, string> = undefined
>(
  reference: REFERENCE,
  fallback?: FALLBACK,
...
export const $sum = <A, B>(a: A, b: B): SUM<A, B> => ({
  [$HAS_VERB]: true,
  [$SUM]: [a, b],
})
...
export const $add = <VALUE>(value: VALUE): ADD<VALUE> => ({
  [$HAS_VERB]: true,
  [$ADD]: value,
})

...
import type { Key, TableV2 } from 'v1/table'

const doesSchemaValidateTableSchemaKey = (
  schema: Schema,
  key?: Key,
): boolean => {
  ...
  const keyAttribute =
    ...
    Object.values(schema.attributes).find(
      attribute => attribute.savedAs === key.name,
    )
  ...
  return (
    ...
    (keyAttribute.required === 'always' ||
      keyAttribute.defaults.key !== undefined)
  )
}
